...
public class LibraryServiceTests
{
    ...
    [Fact]
    public void ShowUsers_ReturnsRegisteredUsers()
    {
        var library = new LibraryService();
        library.RegisterUser("U001", "Laura", "Mendez");
        library.RegisterUser("U002", "Diego", "Suarez");

        var users = library.ShowUsers();

        Assert.Equal(2, users.Length);
        Assert.Contains(users, u => u.Code == "U001" && u.FirstName == "Laura");
        Assert.Contains(users, u => u.Code == "U002" && u.LastName == "Suarez");
    }

    [Fact]
    public void SearchUsers_FindsByFullNameOrCode()
    {
        var library = new LibraryService();
        library.RegisterUser("U001", "Maria", "Zapata");
        library.RegisterUser("U002", "Erick", "Guerron");

        var byName = library.SearchUsers("maria za").ToArray();
        var byCode = library.SearchUsers("u002").ToArray();

        Assert.Single(byName);
        Assert.Equal("U001", byName[0].Code);
        Assert.Single(byCode);
        Assert.Equal("U002", byCode[0].Code);
    }

    [Fact]
    public void ShowBooks_ReturnsAllRegisteredBooks()
    {
        var library = new LibraryService();
        library.RegisterBook("B001", "Cien Anos de Soledad", "Gabriel Garcia Marquez", "Realismo magico", 2);
        library.RegisterBook("B002", "Rayuela", "Julio Cortazar", "Novela", 1);

        var books = library.ShowBooks();

        Assert.Equal(2, books.Length);
        Assert.Contains(books, b => b.Code == "B001");
        Assert.Contains(books, b => b.Code == "B002");
    }

    [Fact]
    public void SearchBooks_FindsByAuthorTitleOrCode_IgnoresCase()
    {
        var library = new LibraryService();
        library.RegisterBook("B001", "Cien Anos de Soledad", "Gabriel Garcia Marquez", "Realismo magico", 2);
        library.RegisterBook("B002", "Rayuela", "Julio Cortazar", "Novela", 1);

        var byAuthor = library.SearchBooks("garcia").ToArray();
        var byTitle = library.SearchBooks("rayu").ToArray();
        var byCode = library.SearchBooks("b001").ToArray();

        Assert.Single(byAuthor);
        Assert.Equal("B001", byAuthor[0].Code);
        Assert.Single(byTitle);
        Assert.Equal("B002", byTitle[0].Code);
        Assert.Single(byCode);
        Assert.Equal("B001", byCode[0].Code);
    }
}
